...
class ListAccountTransactionsTest {
    ...
    private val useCase = ListAccountTransactions(repository)

    @Test
    fun returnsEmptyPageWhenNoTransactions() {
        val result = useCase.execute(ListAccountTransactions.Query(UUID.randomUUID()))
    ...
    fun returnsOnlyTransactionsForAccount() {
        ...
        val result = useCase.execute(ListAccountTransactions.Query(accountId))
    ...
    fun returnsPagedResults() {
        ...
        val result = useCase.execute(
            ListAccountTransactions.Query(accountId, page = 0, pageSize = 3)
        )
