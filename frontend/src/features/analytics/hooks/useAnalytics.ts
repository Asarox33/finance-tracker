...
export function usePortfolioValue(currency = "EUR") {
    const asOf = today();
    const { data, error, isLoading } = useSWR(["portfolio-value", asOf, currency], () =>
...
export function usePerformance(currency = "EUR", months = 12) {
    ...
    const to = today();
    const { data, error, isLoading } = useSWR(["performance", from, to, currency], () =>
...
export function usePerformanceAfterFees(currency = "EUR", months = 12) {
    const from = monthsAgo(months);
    const to = today();
    const { data, error, isLoading } = useSWR(["performance-fees", from, to, currency], () =>
...
export function usePerformanceAfterInflation(currency = "EUR", months = 12) {
    const from = monthsAgo(months);
    const to = today();
    const { data, error, isLoading } = useSWR(["performance-inflation", from, to, currency], () =>
