...
describe("institutionsApi", () => {
    ...
    it("lists institutions with name filter", async () => {
        ...
        expect(mockFetch).toHaveBeenCalledWith(expect.stringContaining("name=BNP"), expect.anything());
    });
    ...
    it("gets a single institution by id", async () => {
        ...
        expect(mockFetch).toHaveBeenCalledWith(expect.stringContaining("/institutions/inst-1"), expect.anything());
    });
