...
import { Card, Skeleton, ErrorState, PageHeader, Badge } from "@/shared/components/ui";
import { formatMoney, formatBasisPoints, formatDate } from "@/lib/format";
...
export default function AnalyticsPage() {
  ...
  return (
    <div className={styles.page}>
      <PageHeader
        title="Analytics"
        description="Performance analysis and portfolio insights"
      />
              ...
              <table aria-label="Performance breakdown">
                <thead>
                  <tr>
                    ...
                    <th scope="col" style={{ textAlign: "right" }}>After Fees</th>
                    <th scope="col" style={{ textAlign: "right" }}>After Inflation</th>
                  </tr>
                </thead>
                <tbody>
                  <tr>
                  ...
                  <tr>
                    <td>Gain / Loss</td>
                    <td style={{ textAlign: "right", fontFamily: "var(--font-mono)", color: perf.gainLoss >= 0 ? "var(--success)" : "var(--danger)" }}>
                      {formatMoney(perf.gainLoss, currency)}
                    </td>
                    <td style={{ textAlign: "right", fontFamily: "var(--font-mono)", color: (perfFees?.gainLoss ?? 0) >= 0 ? "var(--success)" : "var(--danger)" }}>
                      {perfFees ? formatMoney(perfFees.gainLoss, currency) : "—"}
                    </td>
                    <td style={{ textAlign: "right", fontFamily: "var(--font-mono)", color: (perfInflation?.gainLoss ?? 0) >= 0 ? "var(--success)" : "var(--danger)" }}>
                      {perfInflation ? formatMoney(perfInflation.gainLoss, currency) : "—"}
                    </td>
                  </tr>
                  <tr>
                    <td>Return</td>
                    <td style={{ textAlign: "right", fontFamily: "var(--font-mono)", color: perf.gainLossBasisPoints >= 0 ? "var(--success)" : "var(--danger)" }}>
                      {formatBasisPoints(perf.gainLossBasisPoints)}
                    </td>
                    <td style={{ textAlign: "right", fontFamily: "var(--font-mono)", color: (perfFees?.gainLossBasisPoints ?? 0) >= 0 ? "var(--success)" : "var(--danger)" }}>
                      {perfFees ? formatBasisPoints(perfFees.gainLossBasisPoints) : "—"}
                    </td>
                    <td style={{ textAlign: "right", fontFamily: "var(--font-mono)", color: (perfInflation?.gainLossBasisPoints ?? 0) >= 0 ? "var(--success)" : "var(--danger)" }}>
    ...
    <Card className={styles.perfCard}>
      ...
      {loading ? (
        <>
          ...
          <Skeleton style={{ height: "1rem", width: "60%", marginTop: "0.5rem" }} />
        </>
      ) : error ? (
        ...
        <>
          ...
          <p className={styles.perfGain}>
            {formatMoney(data?.gainLoss ?? 0, data?.currency ?? "EUR")}
          </p>
        </>
      )}
    </Card>
