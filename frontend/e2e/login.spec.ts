...
test.describe("Auth flows", () => {
  ...
  test("login shows locked warning on locked account", async ({ page }) => {
    await page.route("**/api/auth/login", route =>
        route.fulfill({ status: 429, contentType: "application/json", body: JSON.stringify({ message: "Account is temporarily locked. Try again later." }) })
    );
    await page.goto("/login");
  ...
  test("register redirects to login on success", async ({ page }) => {
    await page.route("**/api/auth/register", route =>
        route.fulfill({ status: 201, contentType: "application/json", body: JSON.stringify({ userId: "new-user-id" }) })
    );
    await page.goto("/login/register");
    await page.getByLabel("Email address").fill("new@example.com");
    await page.getByLabel(/^Password/).first().fill("MyStrongPassword123!");
    await page.getByLabel("Confirm password").fill("MyStrongPassword123!");
    await page.getByRole("button", { name: "Create account" }).click();
    await expect(page).toHaveURL(/\/login\?registered=1/, { timeout: 5000 });
  });
